...
| 2026-04-24 | ADR-007 | Remove legacy custom Maven repository declarations for CI resolution, while deferring runtime updater migration | External repo policy/availability issues and version drift caused unstable CI resolution; updater logic still requires separate URL/listing migration work | CI resolution is stabilized, but runtime update endpoint migration remains a follow-up | Accepted |
