...
| Date | Commit | Change |
|------|--------|--------|
| 2026-05-26 | TBD | Realign the `protect-develop` ruleset required checks from the now-removed `validate (zulu-8)` (deleted with `ci.yml`) to the Phase 0 `Maven CI` jobs (`validate-java8`, `deterministic-tests-java8`, `compile-and-package-java8`), unblocking `develop` PRs stranded at "Expected — Waiting". Sync the committed ruleset payload with the live ruleset (squash-only, `code_scanning`, `code_quality`, `copilot_code_review`) and update the governance, required-checks-roadmap, and repository-settings docs |
