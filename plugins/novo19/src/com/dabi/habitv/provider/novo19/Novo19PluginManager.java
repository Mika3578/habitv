...
public class Novo19PluginManager extends BasePluginWithProxy implements PluginProviderDownloaderInterface {
	...
	public Set<EpisodeDTO> findEpisode(final CategoryDTO category) {
		...
		try {
			...
			CategoryDTO resolvedCategory = ensureContentKind(category, page);
			resolvedCategory = Novo19CatalogMapper.ensureExtensionMatchesContentKind(resolvedCategory);
			final String seasonRailSrc = resolvedCategory.getParameter(Novo19Conf.PARAMETER_SEASON_RAIL_SRC);
			if (!StringUtils.isEmpty(seasonRailSrc)) {
				...
				final Set<EpisodeDTO> episodes = Novo19CatalogMapper.mapEpisodes(resolvedCategory, page, seasonTiles);
				diagnostics.setCreatedItems(episodes.size());
				logDiagnostics(diagnostics);
				return episodes;
			}
			final boolean atProgramRoot = StringUtils.isEmpty(resolvedCategory.getParameter(Novo19Conf.PARAMETER_SEASON_INDEX));
			final List<Novo19Tile> railTiles = shouldLoadEpisodeRails(resolvedCategory, page)
					? loadEpisodeRailTiles(page, diagnostics, atProgramRoot)
					: java.util.Collections.<Novo19Tile>emptyList();
			final Set<EpisodeDTO> episodes = Novo19CatalogMapper.mapEpisodes(resolvedCategory, page, railTiles);
			diagnostics.setCreatedItems(episodes.size());
			logDiagnostics(diagnostics);
			return episodes;
		} catch (final IOException e) {
	...
	private static CategoryDTO ensureContentKind(final CategoryDTO category, final Novo19BffPage page) {
