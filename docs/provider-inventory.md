...
| `canalPlus` (+ embedded D17) | **Obsolete** Canal-era endpoints | Dedicated canal-family rewrite |
...
- **Pluzz** → treat as **France Télévisions / France.tv**; module is
  `plugins/francetv`.
- **D17** → legacy Canal+ channel plugin inside `plugins/canalPlus`, not a
  standalone module.
...
| `plugins/canalPlus` | `canalPlus` | provider | obsolete endpoint | `CanalPlusConf`/`D17Conf` use old Canal service URLs and channel-specific legacy endpoints | rewrite provider |
...
| `canalPlus` (`D17` family) | Local fixture metadata + offline baseline test | Multiple legacy endpoint families, highest rewrite risk |
...
2. **fix(provider-canal-family): rewrite canalPlus + d17 provider endpoints**
