...
| Group | Modules |
|-------|---------|
| Recently worked | `francetv` (ex Pluzz), `youtube` (yt-dlp), `novo19`, `telequebec` |
...
public URLs exist. Télé-Québec discovers shows via public GraphQL; playback is
often Canada-geo-limited and fails closed without bypass. Canal+ family and
WAT/TF1+ are not treated as freely downloadable DRM catalogues.
