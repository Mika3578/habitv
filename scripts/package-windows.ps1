...
try {
    ...
    $packagesBinDir = Join-Path $packagesDir "bin"
    New-Item -ItemType Directory -Force -Path $packagesBinDir | Out-Null
    ...
    Copy-Item -Path (Join-Path $stagingBinDir "habitv.bat") `
        -Destination (Join-Path $packagesBinDir "habitv.bat") -Force
    ...
    $launch4jOutExe = Join-Path $packagesBinDir "Habitv.exe"
    $launch4jContent = @"
