...
| R-006 | Dependency scan bootstrap may be slow/cancelled on first NVD update and may produce no report artifact | High | Medium | High | Keep scan non-blocking, pin plugin version, add cache/timeout and optional NVD API key, warn explicitly when report files are missing | Security Eng | Security workflow runs without artifact or times out during feed update | Open |
