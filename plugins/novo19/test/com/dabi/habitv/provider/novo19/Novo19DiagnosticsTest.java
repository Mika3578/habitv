...
public class Novo19DiagnosticsTest {
	...
	@Test
	public void emptyCatalogDiagnosticIncludesRootCauseAndCounts() {
		final Novo19Diagnostics diagnostics = new Novo19Diagnostics("catalogue");
		diagnostics.setSourcePath("/categories");
		diagnostics.setRootCauseSummary("empty-catalog");
		diagnostics.setCreatedItems(0);
		final String line = diagnostics.formatLogLine();
		assertTrue(line.contains("rootCause=empty-catalog"));
		assertTrue(line.contains("createdItems=0"));
		assertTrue(line.contains("sourcePath=/categories"));
	}
