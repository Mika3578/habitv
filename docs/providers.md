...
| Group | Modules |
|-------|---------|
| Recently worked | `francetv` (ex Pluzz), `youtube` (yt-dlp), `novo19`, `tf1plus` |
...
France.tv, NOVO19, and TF1+ delegate download to the youtube/yt-dlp plugin when
public URLs exist. Canal+ family and legacy WAT HTML listing are not treated as
freely downloadable DRM catalogues. TF1+ exposes TF1/TMC/TFX/TF1 Séries Films/LCI
replay pages only (NOVO19 excluded); geo limits and premium/DRM paths fail clearly.
